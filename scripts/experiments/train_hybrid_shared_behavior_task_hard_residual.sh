...
SEED="${SEED:-1}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/behavior_compression/v1/hybrid_shared_behavior_task_hard_residual_global_two_stage/seed${SEED}}"

if [[ -f "${OUTPUT_DIR}/summary.json" ]]; then
  echo "Skipping completed hybrid residual run: ${OUTPUT_DIR}"
  exit 0
fi
...
python scripts/train_joint_soft_prefix_v2.py \
  ...
  --behavior_residual_joint_lr 0.001 \
  --residual_reparameterization \
  --residual_mode global_matched \
  --residual_bottleneck_size 400 \
  --learning_rate 0.001
